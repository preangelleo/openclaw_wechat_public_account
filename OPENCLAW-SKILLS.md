...
### 2.3 仅仅发推文（发文章）的最简配置要求
如果你**不需要**使用公众号机器人的自动回复功能（不处理用户私信），而**仅仅**只想通过本服务的 `POST /publish` 接口一键发排版好的文章。你只需要准备好：
1. **`wx_appid`**
2. **`wx_secret`**
3. **`openrouter_api_key`**
*注意：仅仅发文章也必须要配置好外网主机的 **IP白名单**，这是由于获取微信 `access_token` 的硬性安全要求。*

### 2.4 微信 Webhook/Callback 配置 (如果需要互动机器人功能)
